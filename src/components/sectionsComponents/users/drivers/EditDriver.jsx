...
const EditDriver = () => {
    ...
    const driver = useSelector(state => state.drivers.find(driver => driver.id === params.id))


    useEffect(() => {
        setTaxiCars(driver.taxiCars)
        setUserTypes(driver.userTypes)
        setAddressesUser(driver.userAddresses)
        setStatuses(driver.statuses)
        setTaxiCar(driver.taxiCar)
        setActiveAddressesUser(driver.activeUserAddresses)
        setUserType(driver.userType)
        setActivityStatus(driver.activityStatus)
        setSurname(driver.surname)
        setPatronymic(driver.patronymic)
        setName(driver.name)
        setUserName(driver.user_name)
        setTotalTrips(driver.totalTrips)
        setBonus(driver.bonus)
        setSelfieChecked(driver.selfieChecked)
        setDriversLicenseChecked(driver.driversLicenseChecked)
        setPassportChecked(driver.passportChecked)
        setNotifyBonuses(driver.notifyBonuses)
        setAva(driver.ava)
        setRemoveCarTaxi(driver.removeCarTaxi)
    }, [driver])
        ...
        <>
            ...
            {taxiCars !== null && <div style={{padding:'15px', border: 'solid 1px #1976d2',borderRadius:8}}>
                ...
                <div style={{padding: '15px 0 15px'}}>
                    <DisableDateInput value={driver.lastTrip} label={'Последняя поездка'}/>
                </div>
                <Divider/>

                <div style={{padding: '15px 0 15px'}}>
                    <DisableDateInput value={driver.created} label={'Создан'}/>
                </div>
                <Divider/>
            </div>}
